# -*- coding: utf-8 -*-
"""
Experiment 10.a: Decoupled Single-Axis with Denser Stride, Huber Loss & LeakyReLU
=================================================================================
Optimizations over Exp 10 Base:
  1. Stride Train reduced from 5 to 2 (2.5x more temporal training data).
  2. Loss changed from MSE to Huber Loss (delta=0.1 rad ~ 5.7 deg) to suppress outlier spikes.
  3. Activation changed from ReLU to LeakyReLU(alpha=0.1) to eliminate dying neuron deadzones.
"""
...
def build_axis_model_10a(window_size=20, input_dim=9, name="model_10a"):
...
def train_single_axis_10a(X_raw, Y_target_angle, axis_name, window_size=20, 
    ...
    # Denser stride = 2 (Option A enhancement over stride 5)
    stride_train = 2
    stride_test = 1
    ...
    model = build_axis_model_10a(window_size, X_raw.shape[1], name=f"m10a_{clean_name}")
...
def run_experiment_10a():
    ...
    output_dir = os.path.join(base_dir, "Navigation-and-Sensor-data-error-minimization-using-AI", 
                              "experiments", "Exp10_Optimizations", "10_a")
    os.makedirs(output_dir, exist_ok=True)
    
    print("=" * 80)
    print("EXPERIMENT 10.a: Decoupled Single-Axis (Stride=2, Huber Loss, LeakyReLU)")
    print("=" * 80)
    ...
    for ds_name in datasets:
        ...
        for axis_name, axis_idx in axes:
            ...
            res = train_single_axis_10a(X_raw, target_angle, axis_name)
            ...
            print(f"Done! RMSE: {res['rmse_rad']:.4f} rad ({res['rmse_deg']:.2f} deg)")
    ...
    csv_path = os.path.join(output_dir, "exp10_a_results.csv")
